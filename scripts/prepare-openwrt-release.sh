...
release="${2:-}"
out_dir="${3:-dist}"
feed_dir="${EDGEPULSE_OPENWRT_FEED_DIR:-../edgepulse-openwrt-feed}"
...
sed -i "s/^PKG_RELEASE:=.*/PKG_RELEASE:=$release/" "$feed_dir/edgepulse/Makefile"
sed -i "s/^PKG_RELEASE:=.*/PKG_RELEASE:=$release/" "$feed_dir/luci-app-edgepulse/Makefile"
